...
img.parentNode.replaceChild(svg,img);};images=document.querySelectorAll(selector);imagesLength=images.length;while(imagesLength--){var _img=images[imagesLength],_imgURL;if(_img.getAttribute('data-src')){_imgURL=_img.getAttribute('data-src')}
else{_imgURL=_img.getAttribute('src')}
if(sortImages[_imgURL]){sortImages[_imgURL].push(_img);}
else{sortImages[_imgURL]=[_img];}}
