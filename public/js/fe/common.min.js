var _ua=(function(u){
	...
	function AndroidSversion(){if(ua.indexOf("Android")>0){var androidversion=parseFloat(ua.slice(ua.indexOf("Android")+8));return androidversion;}}
	function cookie_lang(){var url=location.href;if(url.indexOf("/en/")!=-1){$("body").attr("id","en");}}
	$(function(){
		if(_ua.Mobile){$("meta[name='viewport']").attr('width=device-width, initial-scale=1.0');}
		else{$("meta[name='viewport']").attr('content','width=1100');}
		cookie_lang();location_ani();resize();
		$(window).on('scroll resize',function(){
			var stp=$(window).scrollTop();
			if(stp>180){$("body").addClass('sc');if(sc_tru){TweenLite.to($("body #gheader"),0,{y:-90});sc_tru=false;}TweenLite.to($("body #gheader"),0.3,{y:0});}
			else{sc_tru=true;$("body").removeClass('sc');}
		});
		$(window).resize(function(event){resize();});$(window).load(function(){$("#cover").fadeOut(1200);resize();});
		header();btn_anime();
	});
function location_ani(){
	$('a:not([target="_blank"]):not([href^="#"]):not(.fancybox-close,.fancybox,.closeBtn > a)').each(function(){
		...
		if(target=='_blank'){}
		else{$(this).on('tap',function(){
			var nextUrl=$(this).attr('href');
			$('#cover').fadeIn('600',function(){location.href=nextUrl;$('#cover').delay(1000).fadeOut(1);});
			return false;
		});}
	});
}
var winW, winH;
function resize(){winW=$(window).width();winH=$(window).height();}
var h_animef=true;
function header(){
	var $header=$("#gheader");
	...
	if(!_ua.Mobile){
		$menu_btn.on('mouseover',function(){
			if(h_animef){
				if(!$("#gheader").hasClass('on')){
					$header.addClass('on');header_anime();
				}
			}
		});
		$h_nav_box.on('mouseleave',function(){
			if(h_animef){
				if($("#gheader").hasClass('on')){
					$header.removeClass('on');header_anime_out();
				}
			}
		});
	}
	$(".menu_btn").tap(function(){
		h_animef=false;setTimeout(function(){h_animef=true},1000);
		if($("#gheader").hasClass('on')){$header.removeClass('on');header_anime_out();}
		else{$header.addClass('on');header_anime();}
	});
	$(".col_btn").on('mouseover touchstart',function(){$(this).addClass('on')});
	$(".col_btn").on('tap',function(){$(this).removeClass('on')});
	$(".col_btn").on('mouseleave touchend',function(){$(this).removeClass('on')});
}
...
function btn_anime(){
	...
	$('.live_item').hover(function(){
		...
		$(this).removeClass('on');
		$(this).find('.more').removeClass('on');
		TweenLite.to($(this).find('.entry_title'),0.3,{borderColor:"rgba(225,187,0,0.5)"});
...
var minW=980;var minH=640;
var winW_b=0;var winH_b=0;
var tx=0;var ty=0;var tpt;
var my_hituji={};var tpc=0;
var br_name='';var setElm;
...
function scTs(){
	setElm.each(function(i){
		var setThis=$(this), elmTop=setThis.offset().top, elmHeight=setThis.height(), scrTop=$(window).scrollTop(), winHeight=$(window).height();
